...
function showLoadError(err) {
  ...
  const head = '<div style="font-family: var(--mono); font-size: 10px; letter-spacing: 0.24em; color: var(--gold); margin-bottom: 14px;">— ॐ — UNABLE TO LOAD qna-preview.csv —</div>';
  ...
  const body = isFile
    ...
    : '<p>Make sure <code style="font-family: var(--mono); color: var(--gold);">qna-preview.csv</code> is in the same folder as <code>index.html</code> on the server.</p>';
...
async function loadData() {
  try {
    const resp = await fetch("qna-preview.csv", { cache: "no-cache" });
    if (!resp.ok) throw new Error("Failed to load qna-preview.csv (HTTP " + resp.status + ")");
